...
import useTypeEquipQuery from "../../hooks/Parametrage/useTypeEquip";



function AddRessourceMateriel({ open, onClose, onAddRessourceMateriel }) {
    const { data, isLoading } = useTypeEquipQuery();
    const ressource = data?.data || []; // Access the "data" property inside the response
                ...
                <form onSubmit={handleSubmit}>
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={6}>
                            ...
                            <FormControl fullWidth margin="normal" required>
                                ...
                                <Select labelId="Type_equip-label" name="Type_equip" value={formData.Type_equip} onChange={handleChange}>
                                    {Array.isArray(ressource) && ressource.length > 0 ? (
                                        ressource.map((res) => (
                                            <MenuItem key={res.id} value={res.id_typeequipement}>
                                                {res.libelle}
                                            </MenuItem>
                                        ))
                                    ) : (
                        ...
                        <Grid item xs={6}>
                            ...
                            <TextField fullWidth label="Notes" name="Notes" value={formData.Notes} onChange={handleChange} margin="normal" required />
